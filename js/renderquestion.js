...
renderQuestion.prototype = {
  ...
  render:function(index){
    ...
    this.template.find('.answer').eq(0).find('p').html(question[index].answerA.msg);
    this.template.find('.answer').eq(1).find('p').html(question[index].answerB.msg);
    this.template.find('.answer').eq(2).find('p').html(question[index].answerC.msg);
